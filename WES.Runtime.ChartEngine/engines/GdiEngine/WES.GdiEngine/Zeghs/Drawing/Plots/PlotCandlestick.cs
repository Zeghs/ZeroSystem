...
namespace Zeghs.Drawing.Plots {
	internal sealed class PlotCandlestick : AbstractPlot {
		...
		internal PlotCandlestick(GdiEngine engine, object source, ChartSetting chartSetting)
		...
		public override bool DrawPlot(Layer layer, ChartProperty property, bool onlyUpdateLastBar) {
			...
			if (onlyUpdateLastBar) {
				...
				if (!DrawCandlestick(iOffset, cRectY.Y, cUnit, property.BackgroundColor)) {
					return false;
				}
			} else {
				...
				for (int i = 0; i < iCount; i++) {
					...
					if (iIndex > cAxisX.DataCount) {
						...
						int iOffset = this.ConvertAxisScaleIndexToOffset(iIndex);
						AxisXUnit cUnit = cAxisX.ConvertBarNumberToWidth(iIndex);
						DrawCandlestick(iOffset, cRectY.Y, cUnit, Color.Empty);
					}
				}
		...
		private void DrawTitle(Rectangle rectangle, ChartProperty property) {
			...
			string sTitle = string.Format("{0} {1} ({2}{3}) #{4} {5}", cRequest.Symbol, cSettings.Name, cResolution.Size, cResolution.Type.ToString(), cSettings.Category.ToString(), cRequest.Exchange);
