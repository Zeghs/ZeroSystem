...
namespace Zeghs.Utils {
		...
		/// <summary>
		///   加入string資料(2bytes字串長度 + UTF-8編碼字串)
		/// </summary>
		/// <param name="value">string資料</param>
		/// <returns>返回值: ture=成功, false=失敗</returns>
		public bool Add(string value) {
			byte[] bArray = Encoding.UTF8.GetBytes(value);
			ushort uLength = (ushort) bArray.Length;
			return this.Add(uLength) | this.Add(bArray, 0, uLength);
		}
		...
		/// <summary>
		///   取得string資料(2bytes字串長度 + UTF-8編碼字串)
		/// </summary>
		/// <returns>返回值: string</returns>
		public string GetString() {
			ushort uLength = this.GetUInt16();
			string sText = Encoding.UTF8.GetString(this.Data, this.Position, uLength);
			this.Position += uLength;
