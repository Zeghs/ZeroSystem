...
namespace Taiwan.Rules.Contracts {
	...
	///   台灣選擇權合約規則
	/// </summary>
	[RuleProperty(ERuleType.Contract, "TaiwanIndexOptionContractRule", "Taiwan.Rules.Contracts.TaiwanIndexOptionContractRule", "(台灣指數選擇權合約到期規則)\r\n\r\n台灣指數選擇權各契約結算月份以第三個星期三為最後交易日(如遇國定假日或其他不可抗力因素則以台灣期交所公告為準)。", true)]
	public sealed class TaiwanIndexOptionContractRule : RuleBase, IContractTarget, IContractTime, IRuleSetting {
		private static Dictionary<string, string> __cOptionsTable = new Dictionary<string, string>() {
			{ "TEO", "EXF" }, { "TFO", "FXF" }, { "TXO", "TXF" }
		};
		...
		public TaiwanIndexOptionContractRule() {
			if (ShowSetting() == -1) {
				this.UpdateContractTime(DateTime.UtcNow.AddHours(TaiwanStockExchange.TIME_ZONE));
			}
		}

		public TaiwanIndexOptionContractRule(JToken args) {
			this.args = args;
		...
		public string GetContractTarget(string symbolId) {
			...
			if (sUSymbolId.Length > 3) {
				int iIndex = GetContractIndex(symbolId);
				sRet = string.Format("{0}{1}.tw", GetOptionsTarget(sUSymbolId), iIndex);
			}
			return sRet;
		}
