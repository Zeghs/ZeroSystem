...
namespace Taiwan.Rules.Contracts {
	...
	///   台灣股票期貨合約規則
	/// </summary>
	[RuleProperty(ERuleType.Contract, "TaiwanStockFuturesContractRule", "Taiwan.Rules.Contracts.TaiwanStockFuturesContractRule", "(台灣股票期貨合約到期規則)\r\n\r\n台灣股票期貨各契約結算月份以第三個星期三為最後交易日(如遇國定假日或其他不可抗力因素則以台灣期交所公告為準)。", true)]
	public sealed class TaiwanStockFuturesContractRule : RuleBase, IContractTime, IRuleSetting {
		...
		public TaiwanStockFuturesContractRule() {
			if (ShowSetting() == -1) {
				this.UpdateContractTime(DateTime.UtcNow.AddHours(TaiwanStockExchange.TIME_ZONE));
			}
		}

		public TaiwanStockFuturesContractRule(JToken args) {
			this.args = args;
